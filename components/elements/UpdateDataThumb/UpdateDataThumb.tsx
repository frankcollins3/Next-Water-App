...
export default function UpdateDataThumb() {
    ...
    const updateDailyDataPOST = () => {
      ...
      //  updateDailyData(users_id: ${CURRENT_USER.id}, date: "${DATE}", progress: 50, weekday: "Friday", status: "${STATUS}") {
        weekdayPROMISE
        .then( (weekday) => {
          axios
          .post('/api/graphql', {
    ...
    return (
        <Container id={styles.Cont}>
            <pre className={styles.ghost}> blue</pre>
            {
              ...
              <Container id={styles.Cont}>
                  <pre className={styles.ghost}> blue</pre>
    <img style={{ height: '50px', width: '50px', position: 'relative', top: '-50px' }} id={styles.Img} src={statistics}/>
                  <pre className={styles.ghost}> blue</pre>
                </Container>
                    :
                    <>
