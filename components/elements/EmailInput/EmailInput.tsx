 ...
 function EmailInput (props:any) {
    ...
    const emailinputhandler = (event:any) => {        
        ...
        if (EMAIL_INPUT.includes('.')) {
            let stringAfterPeriod = EMAIL_INPUT.replace(/^.*\.(.*)$/, '$1');            
        }
    }

    const inputfocus = async () => { TOGGLE_INPUT_FOCUS( { payload: 'email' } ) }

    const ghosttext = (event:any) => {
        let target:any = event.target
        let jqtarget:any = $(event.target)
        let targetId:string = event.target.id       
        attributeJQ(target, 'value', '@')         // $(event.target).attr('value', targetId)        
    }
