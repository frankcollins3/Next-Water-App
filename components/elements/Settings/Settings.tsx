...
import {SettingsInterface} from "interfaces/interface"
...
export default function Settings() {
  ...
  const { iPROMISEcookies, setUserSettingsPROMISE } = usePromise()
  ...
  useEffect(() => {
// useEffect for isExample === true:       in such case wherein         every change on settings changes chart immediately. cool feature. 
// set to dispatch so runs every time for any state any check every single state for any change and reset if change.

    
  }, [])
  // }, [props?.isExample === true && dispatch])
      ...
      <>
        ...
        <form onSubmit={handleSubmit}>
          <Container className={styles.formGroupSettings}>
            ...
            />
            <Container style={{ color: "silver" }}>{AGE}</Container>
          </Container>
          <Container className={styles.formGroupSettings}>
