...
export default function LoginSignupForm() {
    ...
    const { loginFunctions } = usePromise();
    const submitFaucetClickLogin = loginFunctions.submitFaucetClickLogin
    ...
    const cookiePROMISE = new Promise( (resolve:any, reject:any) => {
                    ...
                    // would refactor and make 1 object but for now just leaving moving on to other app. 
    ...
    const RENDER = () => {
            ...
            <>
        {
            DISPLAY_FORM === "signup" &&
            <form 
            style={{ cursor: IS_LOGGED_IN === true ? "none" : "" }}
            className={styles.signupLoginForm} id={styles.signupForm}>                         
...
style={{ border: 'none', opacity: PASSWORD_SHOW_CLICK ? "1.0" : PASSWORD_SHOW ? "0.5" : "0.1", 
        ...
        height: '25px', width: '25px', alignSelf: 'center', marginTop: '0.25em' }}
...

                    {
                        ...
                        <img onClick={submitFaucetClickLogin} className={styles.submitFaucet} src={faucet}/>                       
                        {
